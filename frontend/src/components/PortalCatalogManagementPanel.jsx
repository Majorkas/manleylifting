...
const emptyForm = { variantRef: '', handle: '', title: '', priceAmount: '', stockPolicy: 'untracked', sku: '' }
    ...
    <section aria-labelledby="catalog-management-heading" className="mt-8 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <label className="text-sm font-semibold text-slate-700"><span className="mb-1 block">Visibility</span>
          <select aria-label="Visibility" value={isActive == null ? '' : String(isActive)} onChange={(event) => { setIsActive(event.target.value === '' ? undefined : event.target.value); setPage(1) }} className="min-h-11 w-full rounded-md border border-slate-300 px-3 py-2 font-normal">
      ...
      <form onSubmit={submit} className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <h3 className="text-base font-bold text-slate-900">{editingProduct ? 'Edit product' : 'Add a product'}</h3>
            <p className="mt-1 text-sm text-slate-600">Required fields are marked by the browser before saving.</p>
          </div>
          {editingProduct && <button type="button" onClick={() => { setEditingProduct(null); setForm(emptyForm) }} className="min-h-11 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700">Cancel edit</button>}
        </div>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            <label key={field} className="text-sm font-semibold text-slate-700">
              ...
              <input required={['variantRef', 'handle', 'title', 'priceAmount'].includes(field)} value={form[field]} onChange={(event) => setForm((current) => ({ ...current, [field]: event.target.value }))} className="min-h-11 w-full rounded-md border border-slate-300 bg-white px-3 py-2 font-normal" />
            </label>
          ))}
          <label className="text-sm font-semibold text-slate-700"><span className="mb-1 block">Stock policy</span><select aria-label="stockPolicy" value={form.stockPolicy} onChange={(event) => setForm((current) => ({ ...current, stockPolicy: event.target.value }))} className="min-h-11 w-full rounded-md border border-slate-300 bg-white px-3 py-2 font-normal"><option value="untracked">Untracked</option><option value="finite">Finite</option><option value="unavailable">Unavailable</option></select></label>
        </div>
      ...
      {products.length > 0 && (
        <div className="mt-5 overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>{products.map((product) => <tr key={product.id} className="border-b border-slate-100"><td className="px-3 py-3 font-semibold">{product.title}<span className="mt-1 block text-xs font-normal text-slate-500">{product.handle}</span></td><td className="px-3 py-3 font-variant-numeric:tabular-nums">{product.currencyCode} {product.priceAmount}</td><td className="px-3 py-3"><div className="font-variant-numeric:tabular-nums">{product.availableQty} available <span className="text-xs text-slate-500">({product.reservedQty} reserved)</span></div><div className="mt-2 flex flex-wrap gap-1"><input aria-label={`Stock change for ${product.title}`} value={editingProduct?.id === product.id ? stockDelta : ''} onChange={(event) => { setEditingProduct(product); setStockDelta(event.target.value) }} placeholder="+/- qty" className="min-h-10 w-20 rounded border px-2 py-1" /><input aria-label={`Stock reason for ${product.title}`} value={editingProduct?.id === product.id ? stockReason : ''} onChange={(event) => { setEditingProduct(product); setStockReason(event.target.value) }} placeholder="Reason" className="min-h-10 w-28 rounded border px-2 py-1" /><button type="button" onClick={() => mutation.mutate({ productId: product.id, action: 'stock', payload: { delta: stockDelta, reason: stockReason } })} className="min-h-10 rounded bg-slate-100 px-2 py-1 text-xs font-semibold">Adjust stock</button></div></td><td className="px-3 py-3">{product.isActive ? 'Active' : 'Archived'}</td><td className="px-3 py-3"><div className="flex flex-wrap gap-3"><button type="button" onClick={() => { setEditingProduct(product); setForm({ variantRef: product.variantRef, handle: product.handle, title: product.title, priceAmount: product.priceAmount, stockPolicy: product.stockPolicy, sku: product.sku || '' }) }} className="min-h-10 font-semibold text-[#123A7A]">Edit</button><button type="button" onClick={() => mutation.mutate({ productId: product.id, action: 'state', payload: { action: product.isActive ? 'archive' : 'reactivate' } })} className="min-h-10 font-semibold text-[#C61F2A]">{product.isActive ? 'Archive product' : 'Reactivate product'}</button></div></td></tr>)}</tbody>
          </table>
        </div>
      )}
